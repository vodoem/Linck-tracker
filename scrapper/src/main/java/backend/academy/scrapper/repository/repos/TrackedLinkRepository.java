...
public interface TrackedLinkRepository extends JpaRepository<TrackedLink, Long> {
    ...
    @Query("SELECT DISTINCT tl FROM TrackedLink tl " +
    ...
    @Query("SELECT DISTINCT tl FROM TrackedLink tl JOIN FETCH tl.tags t WHERE tl.chat.id = :chatId AND t.name = :tagName")
